...
- Before changing anything under `packages/plugins/@log-company/`, read `DEVELOPMENT_GUIDE.md` completely.
- New Log Company backend features and substantial extensions must follow the Onion Architecture described there: domain and application code stay independent from NocoBase, HTTP, Sequelize, and storage adapters.
- `packages/plugins/@log-company/` is the only source of truth. Never edit installed copies under `storage/plugins/`.
- Keep runtime data (`storage/`, `backups/`, `.env`) out of Git. Do not move or recreate the live Docker data directories as part of source changes.
...
- New upstream NocoBase plugins live under `packages/plugins/@nocobase/plugin-<name>/`. Log Company plugins live under `packages/plugins/@log-company/plugin-<name>/` and follow `DEVELOPMENT_GUIDE.md`. Reuse the existing plugin scaffold; do not invent a parallel package layout.
