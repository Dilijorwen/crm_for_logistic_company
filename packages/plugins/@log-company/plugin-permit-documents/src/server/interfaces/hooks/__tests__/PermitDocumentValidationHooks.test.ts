...
function model(values: Record<string, unknown>, changedFields: string[] = [], previousValues = values): MutableModel {
  return {
    get: (key) => values[key],
    set: (key, value) => {
      values[key] = value;
...
describe('PermitDocumentValidationHooks', () => {
  it('resets managed fields without starting synchronization from a model hook', async () => {
    ...
    await handlers.get('permit_documents.beforeCreate')?.(model(values), {});

    expect(values).toMatchObject({ title: 'DOC-1', sync_status: 'PENDING', external_id: null, status: null });
  ...
  it('restores attempted managed changes for a company-only update', async () => {
    ...
    const previous = { id: '11', title: 'DOC-2', document_type: 'certificate_of_conformity', ...registryValues };
    const values = { ...previous, company_id: '2', status: 'terminated' };
    const updateModel = model(values, ['company_id', 'status'], previous);
    await handlers.get('permit_documents.beforeUpdate')?.(updateModel, {});
  ...
  it('clears old registry data and technical regulations when title changes', async () => {
    ...
    expect(values).toMatchObject({ sync_status: 'PENDING', external_id: null, valid_from: null, status: null });
