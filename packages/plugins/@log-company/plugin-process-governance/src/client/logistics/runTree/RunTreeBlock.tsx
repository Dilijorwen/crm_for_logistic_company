...
import { getInnermostRouteFilterByTk } from '../../processContext/getInnermostRouteFilterByTk';
...
const RUN_COLLECTION = 'transport_runs';
const RUN_HISTORY_COLLECTION = 'transport_run_history';
const MAX_DEPTH = 20;

type ProcessResourceKey = string | number;
export type TreeEntityKind = 'process' | 'run';

type TreeEntityConfig = {
  collection: string;
  historyCollection: string;
  parentRelation: 'parent_processes' | 'parent_runs';
  childRelation: 'child_processes' | 'child_runs';
  historyEntityField: 'process_id' | 'transport_run_id';
  heading: string;
  currentLabel: string;
  missingLabel: string;
...
const TREE_ENTITY_CONFIG: Record<TreeEntityKind, TreeEntityConfig> = {
  process: {
    collection: PROCESS_COLLECTION,
    historyCollection: HISTORY_COLLECTION,
    parentRelation: 'parent_processes',
    childRelation: 'child_processes',
    historyEntityField: 'process_id',
    heading: 'Дерево процессов',
    currentLabel: 'Текущий процесс',
    missingLabel: 'Сначала сохраните процесс, чтобы увидеть дерево связей.',
    inaccessibleLabel: 'Нет доступа к связанному процессу',
    loadError: 'Не удалось загрузить дерево процессов',
  },
  run: {
    collection: RUN_COLLECTION,
    historyCollection: RUN_HISTORY_COLLECTION,
    parentRelation: 'parent_runs',
    childRelation: 'child_runs',
    historyEntityField: 'transport_run_id',
    heading: 'Дерево рейсов',
    currentLabel: 'Текущий рейс',
    missingLabel: 'Сначала сохраните рейс, чтобы увидеть дерево связей.',
    inaccessibleLabel: 'Нет доступа к связанному рейсу',
    loadError: 'Не удалось загрузить дерево рейсов',
  },
};
...
type RunTreeBlockProps = {
  processId?: ProcessResourceKey | null;
  recordId?: ProcessResourceKey | null;
  entityKind?: TreeEntityKind;
...
function isTreeCollection(collectionName: string, collection: unknown, record?: Record<string, unknown> | null) {
  return (getCollectionName(collection) || record?.__collectionName) === collectionName;
}
...
function getKeyFromRecord(
  ...
  const processKey = callMethod(collectionManager, 'getFilterByTK', collectionName, record);
...
function getTreeCollection(collectionManager: unknown, collectionName: string, fallbackCollection?: unknown) {
  return callMethod(collectionManager, 'getCollection', collectionName) || fallbackCollection;
}
...
function useProcessTarget(
  ...
  const processCollection = getTreeCollection(collectionManager, config.collection, collection);
  const formBlockCollection = formBlockContext?.collectionName || dataBlockProps?.collection || collection;
  const isProcessFormBlock = isTreeCollection(config.collection, formBlockCollection);
  ...
  if (formBlockIsUpdate) {
    ...
    for (const candidate of formCandidates) {
      ...
      const key = getKeyFromRecord(collectionManager, config.collection, processCollection, candidate.record);
  ...
  for (const candidate of candidates) {
    if (!isTreeCollection(config.collection, candidate.collection, candidate.record)) {
      continue;
    }
    const key = getKeyFromRecord(collectionManager, config.collection, candidate.collection, candidate.record);
    if (!isEmptyResourceKey(key)) {
      return { status: 'ready', key, source: candidate.source };
    }
  }
  ...
  const hasProcessContext =
    isProcessFormBlock ||
    candidates.some((candidate) => isTreeCollection(config.collection, candidate.collection, candidate.record));
  const isNewProcess =
    formBlockIsCreate ||
    candidates.some(
      (candidate) => candidate.isNew && isTreeCollection(config.collection, candidate.collection, candidate.record),
    );
...
function uniqueSorted(ids: string[], nodes: Map<string, ProcessRecord>) {
  ...
  return unique.sort((left, right) => {
    ...
    const numberDiff =
      Number(leftNode?.process_number || leftNode?.run_number || Number.MAX_SAFE_INTEGER) -
      Number(rightNode?.process_number || rightNode?.run_number || Number.MAX_SAFE_INTEGER);
    if (numberDiff !== 0) {
      return numberDiff;
    }
...
function lastChangedLabel(node: ProcessRecord | undefined, entityKind: TreeEntityKind) {
  if (!node || node.inaccessible) {
    return `Последнее изменение: нет доступа к данным ${entityKind === 'run' ? 'рейса' : 'процесса'}`;
  }
...
function nodeTitle(node: ProcessRecord | undefined, entityKind: TreeEntityKind, fallback: string): string {
  if (!node) {
    return fallback;
  }
  ...
  const number = node.run_number ? `Рейс №${node.run_number}` : 'Рейс';
  const registrationNumber = node.vehicle?.registration_number;
  return registrationNumber ? `${number} — ${registrationNumber}` : number;
}

function historyNodeLabel(node: ProcessRecord, entityKind: TreeEntityKind): string {
  return entityKind === 'run' ? `Рейс №${node.run_number || node.id}` : node.title || String(node.id);
}
...
function useStatusLabels(collectionName: string) {
  const collectionManager = useCollectionManager();
  return useMemo(() => {
    try {
...
export function resolveTreeRecordUrl(currentUrl: string, recordId: string) {
  ...
  for (let index = 0; index < pathSegments.length - 1; index += 1) {
    ...
    if (segment === 'filterbytk' || segment === 'filter-by-tk' || segment === 'filterbytk[]') {
      pathSegments[index + 1] = encodeURIComponent(recordId);
      url.pathname = `/${pathSegments.join('/')}`;
      return `${url.pathname}${url.search}${url.hash}`;
    }
  }
  if (url.searchParams.has('filterByTk')) {
    url.searchParams.set('filterByTk', recordId);
    return `${url.pathname}${url.search}${url.hash}`;
  }
  url.searchParams.set('filterByTk', recordId);
  return `${url.pathname}${url.search}${url.hash}`;
...
function RunTreeBlockComponent({ processId, recordId, entityKind = 'run', modelContext }: RunTreeBlockProps) {
  const config = TREE_ENTITY_CONFIG[entityKind];
  ...
  const fetchProcess = useCallback(
    async (id: ProcessResourceKey): Promise<ProcessRecord> => {
      const response = await api.resource(config.collection).get(
        {
          filterByTk: id,
    ...
    [api, config.collection, entityKind],
  );

  const fetchRelated = useCallback(
    async (id: string, relation: TreeEntityConfig['parentRelation'] | TreeEntityConfig['childRelation']) => {
      const response = await api.resource(`${config.collection}.${relation}`, id).list(
        {
          pageSize: 100,
          appends: entityKind === 'run' ? ['createdBy', 'updatedBy', 'vehicle'] : ['createdBy', 'updatedBy'],
          fields:
  ...
  const fetchEdgeInfo = useCallback(
    async (parent: ProcessRecord, child: ProcessRecord, childParentCount: number): Promise<EdgeInfo> => {
      try {
        const response = await api.resource(config.historyCollection).list(
          {
            filter: {
              [config.historyEntityField]: child.id,
              event_type: 'parent_added',
            },
            pageSize: 50,
            sort: ['-createdAt', '-id'],
            appends: ['createdBy'],
            fields: [config.historyEntityField, 'event_type', 'old_value', 'new_value', 'createdAt', 'createdBy'],
          },
          { skipNotify: true },
        );
        const rows = getData<HistoryRow[]>(response) || [];
        const expectedLabel = historyNodeLabel(parent, entityKind);
        const exact = rows.find((item) => item.new_value === expectedLabel);
        const normalized = rows.find(
          (item) => normalizeHistoryTitle(item.new_value) === normalizeHistoryTitle(expectedLabel),
        );
    ...
    [api, config.historyCollection, config.historyEntityField, entityKind],
  );
  ...
  const loadTree = useCallback(async () => {
    ...
    const visit = async (id: string, direction: LoadDirection, depth: number, path: string[]) => {
      ...
      if (!nextData.nodes.has(id)) {
        try {
          ...
          addNode({ id, title: config.inaccessibleLabel, inaccessible: true });
          return;
        }
      }
      ...
      if (direction === 'up' || direction === 'both') {
        try {
          const parents = await fetchRelated(id, config.parentRelation);
          ...
          nextData.warnings.push(config.inaccessibleLabel);
        }
      }

      if (direction === 'down' || direction === 'both') {
        try {
          const children = await fetchRelated(id, config.childRelation);
    ...
    config.childRelation,
    config.inaccessibleLabel,
    config.parentRelation,
    fetchEdgeInfo,
    fetchProcess,
    fetchRelated,
    processKey,
  ...
  useEffect(() => {
    ...
    loadTree()
      ...
      .catch((err) => {
        if (processKeyRef.current === processKey) {
          setError(getErrorText(err, config.loadError));
          setLoading(false);
        }
      });
  }, [config.loadError, loadTree, processKey]);
  ...
  const navigateToProcess = useCallback(
    (id: string) => {
      ...
      const url = typeof window === 'undefined' ? '' : resolveTreeRecordUrl(window.location.href, id);
      setTreeData(null);
      setError(null);
      setCollapsed({});
      if (url) {
  ...
  const renderCard = useCallback(
    (id: string, options: { current?: boolean; hasChildren?: boolean; forceExpanded?: boolean } = {}) => {
      ...
      const parentTitles = parents
        .map((parentId) => nodeTitle(treeData?.nodes.get(parentId), entityKind, ''))
        .filter(Boolean);
      return (
        <div className="process-tree-row" key={`row-${id}`}>
          ...
          <button
            ...
            disabled={options.current || node?.inaccessible}
          >
            <div className="process-tree-title">
              <span>{nodeTitle(node, entityKind, config.inaccessibleLabel)}</span>
  ...
  if (processTarget.status !== 'ready') {
    return <Alert type="info" showIcon message={config.missingLabel} />;
  }
    ...
    <div className={blockClass} style={themeStyle}>
      ...
      {treeData ? (
        <div className="process-tree-shell">
          <div className="process-tree-heading">{config.heading}</div>
...
export const RunTreeBlock = observer(RunTreeBlockComponent, {
  displayName: 'RunTreeBlock',
});
